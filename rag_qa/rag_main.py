...
from core.new_rag_system import RAGSystem  # 使用新版RAGSystem（支持Agent模式）
from openai import OpenAI # 使用 OpenAI 接口

conf = Config()

def main(query_mode=True, directory_path="data", agent_mode=False):
    ...
    if not query_mode:
        ...
        try:
            rag_system = RAGSystem(vector_store, call_dashscope, agent_mode=agent_mode)
        except Exception as e:
        ...
        while True:
            ...
            try:
                ...
                # 记录完整的答案（用于调试）
                logger.info(f"查询处理完成，问题: '{query[:50]}...'，答案长度: {len(full_answer)}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--agent-mode', action='store_true', help='Enable Agent mode for intelligent query processing.')
    args = parser.parse_args()
    main(query_mode=(not args.data_processing), directory_path=args.data_dir, agent_mode=args.agent_mode)